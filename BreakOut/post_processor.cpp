	...
	: PostProcessingShader(shader), Texture(), Width(width), Height(height), Confuse(false), Chaos(false), Shake(false)
{
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, this->Texture.ID, 0); // 将纹理作为颜色附件附加到帧缓冲
	if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
...
{
	glBindFramebuffer(GL_READ_FRAMEBUFFER, this->MSFBO);//读
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, this->FBO);//写
